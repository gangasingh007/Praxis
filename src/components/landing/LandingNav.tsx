...
export function LandingNav({ session }: LandingNavProps) {
  ...
  return (
    ...
    >
      {/* Floating pill container when scrolled */}
      <div
        className={cn(
          "transition-all duration-500",
          scrolled
            ? "mx-4 sm:mx-6 lg:mx-auto max-w-6xl bg-background/80 backdrop-blur-xl border border-border/50 rounded-2xl shadow-lg shadow-black/5 px-6 py-0.5"
            ...
            <AnimatePresence>
              {navLinks.map((link) => (
                ...
                >
                  <span
                    className={cn(
                      "relative z-10 font-mono text-xs uppercase tracking-[0.15em] font-bold transition-colors duration-200",
            ...
            <div className="hidden sm:flex items-center gap-3">
              {isAuthenticated ? (
                <Link href="/planner">
                  <Button
                    size="sm"
                    className="h-9 px-5 gap-2 font-mono text-xs uppercase tracking-widest font-extrabold rounded-xl cursor-pointer
                ...
                <>
                  <Link href="/login">
                    <Button
                      variant="ghost"
                      size="sm"
                      className="font-mono text-xs uppercase tracking-widest font-bold text-muted-foreground hover:text-foreground cursor-pointer"
                    >
                      Sign In
                    </Button>
                  </Link>
              ...
              <Sheet>
                <SheetTrigger asChild>
                  <Button
                    variant="outline"
                    size="icon"
                    className="bg-background shrink-0 hover:bg-primary/5 hover:text-primary transition-colors cursor-pointer"
                  >
                    <Menu size={20} />
                  </Button>
                </SheetTrigger>
                    ...
                    {navLinks.map((link, idx) => (
                      ...
                      >
                        <SheetClose asChild>
                          <Link
                            href={getHref(link.href)}
                            className="flex items-center justify-between group py-3 px-2 rounded-lg hover:bg-primary/5 transition-all duration-200 cursor-pointer"
                          >
                  ...
                  <div className="p-6 border-t border-border/40 space-y-4">
                    {isAuthenticated ? (
                      <Link href="/planner" className="block w-full">
                        <Button className="w-full h-12 font-mono uppercase tracking-widest font-extrabold rounded-xl shadow-primary/20 text-background cursor-pointer">
                          Go to Dashboard
                      ...
                      <>
                        ...
                        <Link href="/login" className="block w-full">
                          <Button
                            variant="outline"
                            className="w-full h-12 font-mono uppercase tracking-widest font-bold rounded-xl border-border/60 hover:bg-primary/5 transition-colors cursor-pointer"
                          >
                            Sign In
                          </Button>
                        </Link>
